...
<!-- 		            </div> -->
	...
	<div class="row" id="inicio">		
	        ...
	        <div class="ibox">
	            <div>
                    <div class="file-manager">
						...
						<h3 class="mt-3">Visualizar Pastas</h3>
						<p class="text-italic">Última atualização: {{getLastRefreshTime()}}</p>
						<table class="table table-hover table-sm">
							<template v-for="g in filtrados">
								<tr :id="['pasta-' + g.grupoOrdem]" class="cursor-pointer" 
									v-if="g.grupoCounterUser > 0 || g.grupoCounterLota > 0" :key="g.grupoOrdem"
									data-toggle="collapse" :data-target="['#collapsetab-' + g.grupoOrdem]"
									:class="g.grupoCollapsed ? 'bg-light' : 'bg-info'" 
									...
									<td class="px-0">
										<h5 class="font-weight-normal">{{g.grupoNome}}</h5>
										<span id="radioBtn" class="p-0 btn-group btn-sm" 
        ...
        <div class="col-lg-9 animated fadeInRight">
			<div class="container-fluid p-1 mail-box-header" style="overflow: hidden">
						...
						<template v-for="g in filtrados">	
							<div v-if="g.grupoCounterUser > 0 || g.grupoCounterLota > 0" :key="g.grupoOrdem">
								<div class="collapse-header">
									<h4 :id="['collapse-header-' + g.grupoOrdem]" data-toggle="collapse" :data-target="['#collapsetab-' + g.grupoOrdem]" 
											class="collapse-toggle p-1 mb-1 table-group table-group-title"
											v-bind:class="{ collapsed: g.grupoCollapsed}" v-bind:aria-expanded="{false: g.grupoCollapsed}"
											@click="collapseGrupo(g.grupoOrdem, g.grupoNome)">
										<i :class="g.grupoIcone"></i>
										<span class="font-weight-normal mr-2">{{g.grupoNome}}</span>
										<span id="radioBtn" class="p-0 btn-group btn-sm" 
												title="Qtd. de documentos visualizados/total do usuário ou lotação">
											<a class="badge-mesa btn btn-primary btn-sm" v-bind:class="exibeLota ? 'notActive' : 'active'" id="btnUser"  
												accesskey="u" @click="carregarMesaUser('#btnUser');">
								...
								<div :id="['collapsetab-' + g.grupoOrdem]" class="collapse" :key="g.grupoOrdem" v-bind:class="{show: !g.grupoCollapsed }">
									...
									<table v-else class="table table-sm table-striped table-hover table-email table-borderless">
										...
										<tbody>
											<template v-for="f in g.grupoDocs">
												<tr class="d-flex">
													...
													<td class="col-3 d-none d-md-block p-0">
	...
	var appMesa = new Vue({
		  ...
		  methods: {
		    ...
		    collapseGrupo: function(grupoOrdem, grupoNome) {
		    	...
		    	var pastaGrupo = document.getElementById('pasta-' + grupoOrdem);
		  	    if (collapsibleElemHeader.classList.contains('collapsed')) {
		    		setValueGrupo(grupoNome, 'collapsed', false);
		    		pastaGrupo.classList.add('bg-info');
		    		pastaGrupo.classList.remove('bg-light');
		    		...
		    		pastaGrupo.classList.add('bg-light');
		    		pastaGrupo.classList.remove('bg-info');
		    		setValueGrupo(grupoNome, 'collapsed', true);
		    	}
		    },
// 		    getValueGrupo: function(grupoNome, key) {
// 			  	  var parmGrupos = JSON.parse(getParmUser('gruposMesa'));
// 			  	  return parmGrupos[grupoNome] [key];
// 			},
